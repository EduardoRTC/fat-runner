# Fat Runner – Pygame (v3.7 “ground-only” + repeated bosses + 20k difficulty boosts)
import os
import sys
import random
...
CFG = {
    ...
    "boss": {
        ...
        "hp": 3,
        "size": (100, 100),
        "spawn_dist": 15000    # reaparece a cada 20k pixels
    },
    "pw": {
        "size": (28, 28), "int": 6000, "chance": 0.4,
    ...
    "boost": {
        "dist": 20000,        # boost de dificuldade a cada 20k
        "spawn_mult": 0.95,
        "diff_add": 0.05
    }
}
...
class Game:
    def __init__(self):
        ...
        self.state = "menu"
        # próximo spawn de boss e boost de dificuldade
    ...
    def update(self, dt):
        if self.state=="play":
            ...
            # spawn boss a cada 20k (enquanto não houver boss ativo)
            ...
            # ajusta velocidade do boss a cada 10k
    ...
    def draw(self):
        if self.state=="menu":
            ...
            title = self.font.render("FAT RUNNER",True,(255,255,255))
            ...
            self.screen.blit(self.font.render("JOGAR",True,(0,0,0)),self.btn_play.move(60,10))
            self.screen.blit(self.font.render("SAIR",True,(0,0,0)),self.btn_exit.move(80,10))
            ...
            self.screen.blit(self.bg,(self.bx1,0)); self.screen.blit(self.bg,(self.bx2,0))

            pygame.draw.rect(self.screen,CFG["road"]["col"],(0,GROUND_Y,W,H-GROUND_Y))
            self.road_off = getattr(self,"road_off",0)+mv
            self.road_off %= (CFG["road"]["w"]+CFG["road"]["gap"])
            x=-self.road_off
            while x<W:
                pygame.draw.rect(self.screen,CFG["road"]["stripe"],(x,GROUND_Y+20,CFG["road"]["w"],10))
            ...
            pct=(self.player.weight-CFG["wt"]["min"])/(CFG["wt"]["max"]-CFG["wt"]["min"])
            fill=int(bw*pct)
            col=(0,200,0) if pct<0.6 else ((255,165,0) if pct<0.9 else (255,0,0))
            pygame.draw.rect(self.screen,col,(hx,hy,fill,bh))
            self.screen.blit(self.font.render(f"Peso: {self.player.weight:.1f}kg",True,(255,255,255)),(hx,hy+bh+5))
            ds=self.font.render(f"Distância: {int(self.player.dist_px)} px",True,(255,255,255))
            self.screen.blit(ds,(W-ds.get_width()-20,20))
            if self.player.freeze_ms>0:
                fr=self.font.render("CONGELADO!",True,(0,200,255))
                self.screen.blit(fr,fr.get_rect(center=(W//2,50)))

        else:  # gameover
            self.screen.fill((0,0,0))
            go=self.font.render("GAME OVER",True,(255,0,0))
            go=pygame.transform.scale(go,(go.get_width()*2,go.get_height()*2))
            self.screen.blit(go,go.get_rect(center=(W//2,H//2-100)))
            pygame.draw.rect(self.screen,(0,200,0),self.btn_play)
            pygame.draw.rect(self.screen,(200,0,0),self.btn_exit)
            self.screen.blit(self.font.render("JOGAR NOVAMENTE",True,(0,0,0)),self.btn_play.move(10,10))
            self.screen.blit(self.font.render("SAIR",True,(0,0,0)),self.btn_exit.move(80,10))
